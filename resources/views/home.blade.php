...
<body class="bg-slate-950 text-slate-100 antialiased min-h-screen selection:bg-blue-600 selection:text-white"
    ...
    @if (session('success') || session('error') || session('warning'))
        <div x-data="{ open: true }" x-show="open" x-transition
            class="fixed bottom-5 right-5 z-50 max-w-sm w-full bg-slate-900 border border-slate-800 rounded-2xl p-4 shadow-2xl flex justify-between items-start space-x-3">
            <div class="text-sm">
                @if (session('success'))
                    <span class="text-emerald-500 font-bold">✅ Sukses:</span> <span
                        class="text-slate-300">{{ session('success') }}</span>
                @endif
                @if (session('error'))
                    <span class="text-rose-500 font-bold">⚠️ Gagal:</span> <span
                        class="text-slate-300">{{ session('error') }}</span>
                @endif
                @if (session('warning'))
                    <span class="text-amber-500 font-bold">🔔 Info:</span> <span
                        class="text-slate-300">{{ session('warning') }}</span>
                @endif
            </div>
    ...
    <nav
        class="bg-slate-900/80 backdrop-blur-md border-b border-slate-800/60 sticky top-0 z-40 transition duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
            <a href="#"
                class="flex items-center space-x-2 text-lg font-extrabold tracking-tight text-white group">
            ...
            <div class="flex items-center space-x-4">
                @auth
                    <span
                        class="text-xs text-slate-400 font-medium bg-slate-950 px-3 py-1.5 rounded-lg border border-slate-800 hidden sm:inline-block">
                        Halo, <span class="text-slate-200 font-semibold">{{ Auth::user()->name }}</span>
                    </span>
                    <form action="{{ route('logout') }}" method="POST" class="inline">
                        @csrf
                        <button type="submit"
                            class="text-xs font-bold text-rose-400 bg-rose-500/10 border border-rose-500/20 px-4 py-2 rounded-full hover:bg-rose-500/20 transition duration-200">
                            Logout
                        </button>
                    </form>
                @else
                    ...
                    <a href="/register"
                        class="text-xs font-bold text-white bg-blue-600 px-5 py-2 rounded-full hover:bg-blue-700 shadow-md shadow-blue-600/20 transition duration-200">
                        Daftar
                    </a>
                @endauth
            </div>
    ...
    <section class="relative bg-slate-950 border-b border-slate-900 py-20 lg:py-28 overflow-hidden">
        ...
        <div class="max-w-4xl mx-auto text-center px-4 sm:px-6 relative z-10 space-y-6">
            <h1 class="text-4xl sm:text-5xl lg:text-6xl font-black text-white tracking-tight leading-none">
                Hunian Nyaman & <span
                    class="bg-gradient-to-r from-blue-400 to-indigo-400 bg-clip-text text-transparent">Strategis</span>
            </h1>
            <p class="text-base sm:text-lg text-slate-300 max-w-xl mx-auto leading-relaxed font-medium">
                Kost Tiga Saudara — Menyediakan fasilitas lengkap, lingkungan aman, dan harga sewa bersahabat di
                kelasnya.
            </p>

            <div class="flex flex-wrap justify-center gap-3 pt-4">
                <a href="#listKamar"
                    class="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white text-sm font-bold rounded-full shadow-lg shadow-blue-600/20 flex items-center space-x-2 transition duration-200">
                    <span>Cari Kamar</span>
                    <span class="text-xs">⬇️</span>
                </a>
                <a href="https://wa.me/{{ $wa_admin ?? '628' }}?text=Halo%20Admin,%20saya%20tertarik%20dengan%20info%20kost"
                    target="_blank"
                    class="px-6 py-3 bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-800 text-sm font-bold rounded-full flex items-center space-x-2 transition duration-200">
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16" id="listKamar">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($kamars as $kamar)
                ...
                @endphp
                <div
                    class="bg-slate-900 border border-slate-800/80 rounded-2xl overflow-hidden flex flex-col justify-between transition duration-300 hover:border-slate-700/80 hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/40 group relative">

                    <span
                        class="absolute top-4 right-4 z-10 text-[10px] font-bold tracking-wider uppercase border px-3 py-1 rounded-full backdrop-blur-md {{ $badgeStyle }}">
                        ● {{ $status }}
                    </span>

                    <div>
                        <div
                            class="aspect-video w-full bg-slate-950 overflow-hidden relative border-b border-slate-950 shadow-inner">
                            @if ($kamar->foto_kamar)
                                <img src="{{ asset('storage/' . $kamar->foto_kamar) }}" alt="Foto Kamar"
                        ...
                        <div class="p-5 space-y-3">
                            <h3
                                class="text-lg font-bold text-white tracking-tight group-hover:text-blue-400 transition">
                            ...
                            <p
                                class="text-xs text-slate-400 leading-relaxed line-clamp-2 bg-slate-950/40 p-3 rounded-xl border border-slate-800/40 italic">
                    ...
                    <div class="p-5 pt-0 mt-auto">
                        ...
                        @else
                            <button disabled
                                class="w-full py-2.5 bg-slate-950 text-slate-600 border border-slate-900 text-xs font-bold rounded-xl cursor-not-allowed text-center">
                                Sudah Terisi
                            </button>
                        @endif
                    </div>
            ...
            @empty
                <div
                    class="col-span-full bg-slate-900 border border-dashed border-slate-800 rounded-2xl p-12 text-center text-slate-500 italic text-sm">
    ...
    @auth
        <div x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm"
            ...
            <div @click.away="showModal = false"
                class="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl overflow-hidden shadow-2xl transform transition-all p-6 space-y-4">
                ...
                <form action="{{ route('pengunjung.booking.store') }}" method="POST" class="space-y-4">
                    ...
                    <p class="text-xs text-slate-400 leading-relaxed">
                        Halo <strong>{{ Auth::user()->name }}</strong>, silakan lengkapi parameter berkas berikut untuk
                        mengajukan sewa kontrakan.
                    </p>
                    ...
                    <div class="flex items-center justify-end space-x-2 pt-3 border-t border-slate-800">
                        ...
                        <button type="submit"
                            class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-full shadow-md shadow-blue-600/10 transition">Kirim
                            Pengajuan 🚀</button>
                    </div>
                </form>
            </div>
