...
<div class="space-y-6"
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4">
        <div>
            ...
            <p class="text-slate-400 text-xs font-medium mt-1">Sirkulasi tagihan bulanan massal, validasi kas masuk, dan audit bukti transfer penyewa.</p>
        </div>
    ...
    <div class="bg-slate-900 border border-slate-800/60 rounded-2xl p-4 shadow-sm">
        <form method="GET" class="flex flex-col md:flex-row md:items-end gap-3">
            <div class="w-full md:w-1/4">
                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Status Pembayaran</label>
                <select name="status" class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-2.5 text-slate-200 focus:outline-none focus:border-blue-500 [color-scheme:dark] transition">
            ...
            <div class="w-full md:w-1/4">
                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Bulan Tagihan</label>
            ...
            <div class="flex items-center space-x-2 pt-2 md:pt-0">
                <button type="submit" class="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 text-xs font-bold rounded-xl transition cursor-pointer">
                    🔍 Filter
                </button>
                @if(request('status') || request('bulan'))
                    <a href="{{ route('admin.tagihan.index') }}" class="px-4 py-2.5 bg-slate-950 hover:bg-slate-900 border border-slate-800 text-slate-400 hover:text-slate-200 text-xs font-bold rounded-xl transition">
                        🔄 Reset
                    </a>
                @endif
            </div>
                        ...
                        <tr class="hover:bg-slate-800/20 text-slate-200 transition">
                            ...
                            <td class="px-5 py-3.5">
                                ...
                                @elseif($status === 'menunggu_validasi')
                                    <span class="px-2.5 py-0.5 bg-amber-500/10 text-amber-400 border border-amber-500/20 text-[10px] font-bold rounded-full uppercase tracking-wide animate-pulse">
                                        Perlu Cek
                                    </span>
                                @else
                            ...
                            <td class="px-5 py-3.5 text-center flex justify-center items-center space-x-1">
                                @if($status === 'menunggu_validasi')
                                    <form action="{{ route('admin.tagihan.validate', $row->id_tagihan) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="button" class="btn-validasi p-1.5 text-emerald-400 hover:text-white bg-emerald-500/10 hover:bg-emerald-500 border border-emerald-500/20 rounded-xl cursor-pointer transition">
                                            ✓
                                        </button>
                                    </form>
                                    <form action="{{ route('admin.tagihan.reject', $row->id_tagihan) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="button" class="btn-tolak p-1.5 text-rose-400 hover:text-white bg-rose-500/10 hover:bg-rose-500 border border-rose-500/20 rounded-xl cursor-pointer transition">
                                            ✕
                                        </button>
                                    </form>
                                @elseif($status === 'belum_bayar')
                                    <form action="{{ route('admin.tagihan.validate', $row->id_tagihan) }}" method="POST" class="inline">
                                        @csrf @method('PATCH')
                                        <button type="button" class="btn-validasi px-3 py-1 text-[10px] font-bold text-slate-300 hover:text-white bg-slate-950 border border-slate-800 hover:border-slate-700 rounded-lg cursor-pointer transition">
                                            Set Lunas
                                        </button>
                                    </form>
                                @else
    ...
    <div x-show="showPreviewModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm" x-cloak x-transition>
        <div @click.away="showPreviewModal = false" class="bg-slate-900 border border-slate-800 w-full max-w-xl rounded-2xl overflow-hidden shadow-2xl transform transition-all p-5 space-y-4">
            <div class="flex justify-between items-center border-b border-slate-800/80 pb-2.5">
                <h5 class="text-xs font-extrabold text-white flex items-center space-x-2">
                    <span>🖼️</span> <span>Bukti Transfer:</span> <span x-text="previewNama" class="text-blue-400"></span>
                </h5>
                <button @click="showPreviewModal = false" class="text-slate-400 hover:text-white text-lg font-bold focus:outline-none">&times;</button>
            </div>
            
            <div class="bg-slate-950 rounded-xl p-3 border border-slate-900 flex justify-center items-center shadow-inner overflow-hidden max-h-[70vh]">
                <img :src="previewImg" class="max-w-full max-h-[60vh] object-contain rounded-lg shadow-2xl" alt="Bukti Upload">
            </div>
        </div>
    </div>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        if(btnGen) {
            btnGen.addEventListener('click', () => {
                ...
                }).then((res) => { if(res.isConfirmed) document.getElementById('formGenerate').submit(); });
            });
        }
        ...
        document.addEventListener('click', (e) => {
            ...
            if(actValidate) {
                ...
                Swal.fire({
                    ...
                    color: '#fff',
                    customClass: { popup: 'border border-slate-800 rounded-2xl' }
                }).then((res) => { if(res.isConfirmed) actValidate.closest('form').submit(); });
            }
            ...
            if(actReject) {
                ...
                Swal.fire({
                    ...
                    color: '#fff',
                    customClass: { popup: 'border border-slate-800 rounded-2xl' }
                }).then((res) => { if(res.isConfirmed) actReject.closest('form').submit(); });
            }
        });
    });
</script>
