    ...
    <div class="space-y-6" x-data="{
        ...
        <div class="flex flex-col md:flex-row justify-between items-center gap-4">
            <div class="text-center md:text-left w-full">
                ...
                <p class="text-slate-400 text-xs font-medium mt-1">Kelola inventaris, harga sewa bulanan, dan berkas foto
                    fasilitas kamar kost.</p>
            </div>

            <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto shrink-0">
                <div class="relative w-full sm:w-64">
                    <span class="absolute inset-y-0 left-0 flex items-center pl-3">
                        <img src="{{ asset('assets/icon/search.png') }}" alt="Search"
                            class="w-3.5 h-3.5 object-contain opacity-50">
                    </span>
        ...
        {{-- Notifikasi saat validasi error / proses database gagal --}}
                        ...
                        @forelse($kamars as $row)
                            ...
                            <tr x-show="search === '' || '{{ strtolower($row->no_kamar . ' ' . $row->deskripsi) }}'.includes(search.toLowerCase())"
                                ...
                                <td class="px-5 py-3.5 text-center space-x-1 flex justify-center items-center">
                                    ...
                                    <form id="delete-form-{{ $row->id_kamar }}"
                                        action="{{ route('admin.kamar.destroy', $row->id_kamar) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="button" onclick="konfirmasiHapus('{{ $row->id_kamar }}')"
                                            class="p-2 text-rose-400 hover:text-white bg-rose-500/10 hover:bg-rose-500 border border-rose-500/20 rounded-xl cursor-pointer transition">
                                            🗑️
                                        </button>
                                    </form>
                                </td>
        ...
        <div x-show="showAddModal"
            ...
            <div @click.away="showAddModal = false"
                ...
                <form action="{{ route('admin.kamar.store') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Harga
                                Sewa (Rp)</label>
                            <input type="number" name="harga_sewa" required min="0" placeholder="Cth: 500000"
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Fasilitas
                            Kamar</label>
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Foto Kamar
                            (Opsional)</label>
        ...
        <div x-show="showEditModal"
            ...
            <div @click.away="showEditModal = false"
                ...
                <form :action="'/admin/kamar/' + editData.id" method="POST" enctype="multipart/form-data"
                    class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Harga
                                Sewa (Rp)</label>
                            <input type="number" name="harga_sewa" x-model="editData.harga" required min="0"
                                class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                        </div>
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Ganti
                            Foto Kamar (Opsional)</label>
                        <input type="file" name="foto" accept="image/*"
                            class="w-full text-xs text-slate-400 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-bold file:bg-slate-950 file:text-slate-300 file:cursor-pointer hover:file:bg-slate-800 transition">
                        <span class="text-[10px] text-slate-500 block mt-1">Biarkan kosong jika tidak ingin merubah foto
                            lama.</span>
                    </div>
                    <div class="flex items-center justify-end space-x-2 pt-3 border-t border-slate-800">
                        <button type="button" @click="showEditModal = false"
                            class="px-4 py-2 text-xs font-semibold text-slate-400 hover:text-white transition">Batal</button>
                        <button type="submit"
                            class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-full shadow-md shadow-blue-600/10 transition">Update
                            Kamar 💾</button>
                    </div>
                </form>
            </div>
