...
<div class="space-y-6" 
    ...
    <div class="flex flex-col md:flex-row justify-between items-center gap-4">
        <div class="text-center md:text-left w-full">
            ...
            <p class="text-slate-400 text-xs font-medium mt-1">Daftar rekaman penyewa / kontrakan aktif di Tiga Saudara saat ini.</p>
        </div>
                    ...
                    @forelse($penghunis as $row)
                        <tr x-show="search === '' || '{{ strtolower($row->nama_lengkap . ' ' . $row->nomor_kamar) }}'.includes(search.toLowerCase())"
                            ...
                            <td class="px-5 py-3.5 space-y-0.5">
                                ...
                                <div class="text-[10px] text-slate-500 font-mono tracking-wider">NIK: {{ $row->nik_ktp }}</div>
                            </td>
                            ...
                            <td class="px-5 py-3.5">
                                ...
                                @else
                                    <span class="px-2.5 py-0.5 bg-slate-800 text-slate-500 border border-slate-700 text-[10px] font-bold rounded-full uppercase tracking-wide">
                                        Keluar
                                    </span>
                                @endif
                            </td>
                            ...
                            <td class="px-5 py-3.5 text-center space-x-1 flex justify-center items-center">
                                ...
                                <form action="{{ route('admin.penghuni.checkout', $row->id_penghuni) }}" method="POST" class="inline">
                                    @csrf
                                    <button type="button" class="btn-checkout p-2 text-rose-400 hover:text-white bg-rose-500/10 hover:bg-rose-500 border border-rose-500/20 rounded-xl cursor-pointer transition" title="Check-out">
                                        🚪💨
                                    </button>
                                </form>
                            </td>
    ...
    <div x-show="showAddModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" x-cloak x-transition>
        <div @click.away="showAddModal = false" class="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl transform transition-all p-6 space-y-4">
            ...
            <form action="{{ route('admin.penghuni.store') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-3">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                        </div>
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Nomor NIK KTP</label>
                            <input type="number" name="nik" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">No. HP (WhatsApp)</label>
                                <input type="text" name="hp" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                            </div>
                            <div>
                                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Email Akun</label>
                                <input type="email" name="email" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3 md:border-l md:border-slate-800/80 md:pl-6">
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Pilih Blok Kamar</label>
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Tanggal Masuk</label>
                        ...
                        <div class="pt-2">
                            <div class="p-3.5 bg-slate-950/60 rounded-xl border border-slate-800 text-[10px] text-slate-500 leading-relaxed font-medium">
                                🔐 <strong>Pengamanan Kredensial:</strong> Sistem otomatis akan membuatkan password default berupa <strong class="text-slate-300">123456</strong> untuk akun login perdana penghuni baru ini.
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end space-x-2 pt-3 border-t border-slate-800">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold rounded-full shadow-md shadow-blue-600/10 transition">Daftarkan Penghuni 🚀</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <div x-show="showEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" x-cloak x-transition>
        <div @click.away="showEditModal = false" class="bg-slate-900 border border-slate-800 w-full max-w-2xl rounded-2xl overflow-hidden shadow-2xl transform transition-all p-6 space-y-4">
            <div class="flex justify-between items-center border-b border-slate-800 pb-3">
                <h5 class="text-base font-extrabold text-white flex items-center space-x-2">
            ...
            <form :action="'/admin/penghuni/' + editForm.id_penghuni" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-3">
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Nama Lengkap</label>
                            <input type="text" name="nama" x-model="editForm.nama" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                        </div>
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Nomor NIK KTP</label>
                            <input type="number" name="nik" x-model="editForm.nik" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                        </div>
                        <div class="grid grid-cols-2 gap-2">
                            <div>
                                <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">No. HP (WhatsApp)</label>
                                <input type="text" name="hp" x-model="editForm.hp" required class="w-full text-xs bg-slate-950 border border-slate-800 rounded-xl p-3 text-slate-200 focus:outline-none focus:border-blue-500 transition">
                            </div>
                            <div>
                                <label class="block text-[11px] font-bold text-slate-500 uppercase tracking-wider mb-1.5">Email (Terikat)</label>
                                <input type="email" x-model="editForm.email" readonly class="w-full text-xs bg-slate-950 border border-slate-800/40 rounded-xl p-3 text-slate-500 cursor-not-allowed select-none focus:outline-none">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3 md:border-l md:border-slate-800/80 md:pl-6">
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Kamar Hunian (Opsi Pindah)</label>
                        ...
                        <div>
                            <label class="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">Tanggal Masuk / Mutasi Kamar</label>
                            <input type="date" name="tgl_masuk" x-model="editForm.tanggal_tampil" required readonly
                            ...
                            <span class="text-[10px] text-slate-500 block mt-1.5 leading-normal">
                                *Jika memutuskan mutasi pindah kamar, parameter tanggal sewa otomatis dimutasi ke <span class="text-amber-500 font-bold">Hari Ini</span> untuk penyesuaian siklus tagihan baru.
                            </span>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex items-center justify-end space-x-2 pt-3 border-t border-slate-800">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-amber-500 hover:bg-amber-600 text-slate-950 text-xs font-black rounded-full shadow-md shadow-amber-500/10 transition">Update Data 💾</button>
                </div>
            </form>
        </div>
    </div>
